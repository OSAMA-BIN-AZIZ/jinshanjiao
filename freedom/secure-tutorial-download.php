        ...
        <div class="wrap">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" style="max-width:1000px">
                ...
                <table class="form-table">
                    ...
                    <tr><th>售价</th><td><input name="payment_amount" type="number" min="0" step="0.01" value="<?php echo esc_attr($options['payment_amount']); ?>" style="width:160px;min-width:160px;font-size:16px;padding:6px 10px;"> <select name="payment_currency" style="min-width:145px;font-size:15px;"><?php foreach(['CNY'=>'CNY 人民币','USD'=>'USD 美元','EUR'=>'EUR 欧元'] as $c=>$l): ?><option value="<?php echo esc_attr($c); ?>" <?php selected($options['payment_currency'], $c); ?>><?php echo esc_html($l); ?></option><?php endforeach; ?></select></td></tr>
...
<div class="card"><h3>Freedom 客服</h3><p>右下角 Freedom 客服会按电脑、Android、小火箭三个教程回答。不要把完整订阅链接、节点 URL、TXT 内容或账号密码发送给客服。</p></div>
        ...
        <div class="modal" id="pwmodal"><div class="dialog"><h3>安全下载验证</h3><p id="pwlabel"></p><input class="field" id="pwinput" type="text" placeholder="请输入下载密码（输入内容可见）" autocomplete="off" autocapitalize="off" spellcheck="false"><div class="msg" id="pwmsg" role="status" aria-live="polite"></div><div class="copy-result" id="pwcopy" hidden><div class="copy-result-head"><strong data-copy-title>订阅 / 节点内容</strong><button type="button" class="freedom-copy-btn" data-copy-button data-copy-source="pwcopytext">点击拷贝</button></div><textarea class="copy-content" id="pwcopytext" rows="7" readonly spellcheck="false" aria-label="订阅或节点内容"></textarea><div class="msg" data-copy-status role="status" aria-live="polite"></div></div><div class="freedom-download-actions"><button type="button" class="freedom-modal-btn freedom-modal-btn-secondary" id="pwcancel">取消</button><button type="button" class="freedom-modal-btn freedom-modal-btn-primary" id="pwsubmit">确认并下载</button></div></div></div>
        <script>
        (function(){
