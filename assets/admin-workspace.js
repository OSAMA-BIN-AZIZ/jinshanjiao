(function(){
    ...
    function renderProfile(conversation) {
        ...
        const statusOptions = ['new:新客户','contacted:已联系','qualified:有意向','closed:已成交/关闭'].map(function(item){ const parts = item.split(':'); return '<option value="' + parts[0] + '"' + (conversation.status === parts[0] ? ' selected' : '') + '>' + parts[1] + '</option>'; }).join('');
