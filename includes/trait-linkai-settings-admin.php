...
trait LinkAI_Settings_Admin
{
    ...
    public static function sanitize_options(array $input): array
    {
        ...
        return [
            ...
            'widget_launcher_text' => isset($input['widget_launcher_text']) ? sanitize_text_field($input['widget_launcher_text']) : $defaults['widget_launcher_text'],
            'widget_primary_color' => isset($input['widget_primary_color']) ? sanitize_hex_color($input['widget_primary_color']) ?: $defaults['widget_primary_color'] : $defaults['widget_primary_color'],
            'widget_position' => isset($input['widget_position']) && in_array($input['widget_position'], ['right', 'left'], true) ? $input['widget_position'] : $defaults['widget_position'],
            'departments' => isset($input['departments']) ? self::sanitize_departments_text(wp_unslash($input['departments'])) : $defaults['departments'],
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr>
                        <th scope="row">聊天组件外观</th>
                        <td>
                            <p><label for="linkai-widget-launcher">按钮文案</label><br><input id="linkai-widget-launcher" name="<?php echo esc_attr(self::OPTION_NAME); ?>[widget_launcher_text]" type="text" class="regular-text" value="<?php echo esc_attr($options['widget_launcher_text']); ?>" /></p>
                            <p><label for="linkai-widget-color">主题色</label><br><input id="linkai-widget-color" name="<?php echo esc_attr(self::OPTION_NAME); ?>[widget_primary_color]" type="color" value="<?php echo esc_attr($options['widget_primary_color']); ?>" /></p>
                            <p><label for="linkai-widget-position">位置</label><br><select id="linkai-widget-position" name="<?php echo esc_attr(self::OPTION_NAME); ?>[widget_position]"><option value="right" <?php selected($options['widget_position'], 'right'); ?>>右下角</option><option value="left" <?php selected($options['widget_position'], 'left'); ?>>左下角</option></select></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="linkai-departments">咨询部门</label></th>
                        <td><textarea id="linkai-departments" name="<?php echo esc_attr(self::OPTION_NAME); ?>[departments]" class="regular-text" rows="4"><?php echo esc_textarea($options['departments']); ?></textarea><p class="description">每行一个部门/咨询类型，例如销售、售后、配件咨询；访客发送消息时会带上选择结果，后台会话和 CSV 会同步显示。</p></td>
                    </tr>
                    <tr>
    ...
    private static function sanitize_departments_text(string $departments): string
    {
        $lines = preg_split('/[\r\n,，]+/', $departments);
        $clean = [];
        foreach ($lines ?: [] as $line) {
            $department = trim(sanitize_text_field($line));
            if ($department !== '' && !in_array($department, $clean, true)) {
                $clean[] = function_exists('mb_substr') ? mb_substr($department, 0, 40) : substr($department, 0, 120);
            }
            if (count($clean) >= 20) {
                break;
            }
    ...
    private static function get_departments(): array
    {
        $options = self::get_options();
        $departments = self::sanitize_departments_text((string) ($options['departments'] ?? ''));
        if ($departments === '') {
            return [];
        }
    ...
    private static function sanitize_department(string $department): string
    {
        $department = trim(sanitize_text_field($department));
        $departments = self::get_departments();
        if ($department === '' || empty($departments)) {
            return '';
        }

        return in_array($department, $departments, true) ? $department : $departments[0];
    }
    ...
    private static function default_options(): array
    {
        return [
            ...
            'auto_render' => '1',
            'widget_launcher_text' => '在线客服',
            'widget_primary_color' => '#d71920',
