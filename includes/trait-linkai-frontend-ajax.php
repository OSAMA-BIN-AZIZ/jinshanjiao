...
trait LinkAI_Frontend_Ajax
{
    ...
    public static function handle_satisfaction_request(): void
    {
        ...
        $comment = isset($_POST['comment']) ? substr(sanitize_text_field(wp_unslash($_POST['comment'])), 0, 255) : '';
    ...
    public static function handle_chat_request(): void
    {
        ...
        $message = isset($_POST['message']) ? substr(sanitize_textarea_field(wp_unslash($_POST['message'])), 0, 4000) : '';
        $history_json = isset($_POST['history']) ? wp_unslash($_POST['history']) : '[]';
        $conversation_id = isset($_POST['conversation_id']) ? sanitize_key(wp_unslash($_POST['conversation_id'])) : '';
        $customer_name = isset($_POST['customer_name']) ? substr(sanitize_text_field(wp_unslash($_POST['customer_name'])), 0, 100) : '';
        $contact = isset($_POST['contact']) ? substr(sanitize_text_field(wp_unslash($_POST['contact'])), 0, 120) : '';
        $department = isset($_POST['department']) ? self::sanitize_department(wp_unslash($_POST['department'])) : '';
        if ($conversation_id === '') {
        ...
        if (empty($reception_state['is_online'])) {
            $pause_state = self::record_customer_question($conversation_id, $customer_name, $contact, $department, $message);
            wp_send_json_success([
        ...
        foreach (array_slice($history, -8) as $item) {
            ...
            $content = trim(substr(sanitize_textarea_field((string) $item['content']), 0, 2000));
            if ($content !== '') {
    ...
    private static function record_customer_question(string $conversation_id, string $customer_name, string $contact, string $department, string $message): array
    {
        global $wpdb;
        self::create_customer_tables();
        self::maybe_upgrade_customer_tables();
        ...
        if ($customer) {
            ...
            $wpdb->update(
                ...
                [
                    ...
                    'department' => $department !== '' ? $department : ($customer->department ?? ''),
                    'ip_address' => self::get_customer_ip(),
                    'country' => self::get_customer_country(),
                    'device' => self::get_customer_device(),
                    'user_agent' => self::get_customer_user_agent(),
