...
trait LinkAI_Export
{
    public static function handle_export_customers_request(): void
    {
        ...
        $customers = $wpdb->get_results("SELECT conversation_id, customer_name, contact, status, priority, tags, follow_up_at, assigned_user_id, ai_paused, unread_count, last_message, last_reply, ip_address, country, device, created_at, updated_at FROM " . self::customers_table() . " ORDER BY updated_at DESC LIMIT 5000", ARRAY_A);

        self::send_csv_headers('linkai-customers-' . gmdate('Ymd-His') . '.csv');
        $output = fopen('php://output', 'w');
        fputcsv($output, ['会话ID', '姓名', '联系方式', '跟进状态', '优先级', '标签', '下次跟进', '负责客服', 'AI暂停', '未读数', '最后咨询', '最后回复', 'IP', '国家', '设备', '创建时间', '更新时间']);
